...
public class WDispatcherServlet extends HttpServlet {
    ...
    private List<WHandlerMapping> handlerMappings = new ArrayList<WHandlerMapping>();

    private Map<WHandlerMapping,WHandlerAdapter> handlerAdapters = new HashMap<WHandlerMapping,WHandlerAdapter>();
...
//            new GPModelAndView("500");
    ...
    private WHandlerAdapter getHandlerAdapter(WHandlerMapping handler) {
        if(this.handlerAdapters.isEmpty()){return null;}
        WHandlerAdapter ha = this.handlerAdapters.get(handler);
    ...
    private WHandlerMapping getHandler(HttpServletRequest req) throws Exception{
        ...
        for (WHandlerMapping handler : this.handlerMappings) {
            try{
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        context = new WApplicationContext(config.getInitParameter(CONTEXT_CONFIG_LOCATION));
    ...
    //初始化策略
    protected void initStrategies(WApplicationContext context) {
        //多文件上传的组件
    ...
    private void initViewResolvers(WApplicationContext context) {
        ...
        for (int i = 0; i < templates.length; i ++) {
            ...
            //在我写的代码中简化了，其实只有需要一个模板就可以搞定
             //只是为了仿真，所有还是搞了个List
            this.viewResolvers.add(new WViewResolver(templateRoot));
        }
        ...
        try {
            ...
            for (String beanName : beanNames) {
                ...
                for (Method method : methods) {
                    ...
                    this.handlerMappings.add(new WHandlerMapping(pattern,controller,method));
